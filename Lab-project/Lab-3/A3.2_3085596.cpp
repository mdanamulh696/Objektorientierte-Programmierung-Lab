...
// 3.(A3.1) overloded operator function
...
ostream& operator<<(ostream& out, Year& year )
{
    ...
    out<<"year"<<": "<<year.yearValue<<"( unit: "<<year.unit<<" )"<<endl;
    for(int i=0; i<days_in_year; i++)
     {
        int q_h=0;
...
// A3.2 task 3... function overloaded binary subtraction operator- 
Year operator-(Year& year1, Year& year2)
{
    ...
    {
        cout<<"not machting"<<endl;
        return Year();
    }

    // new year struct create  korlam values store korar jonno
    Year year_new;
    year_new.yearValue = year1.yearValue;
    year_new.firstDayOfWeek = year1.firstDayOfWeek;
    year_new.unit = year1.unit;

    // sub element by element
    for(int i =0; i<days_in_year; i++ )
    {
        for(int j=0; j<interval_quater_hour; j++)
        {
            year_new.values[i][j] = year1.values[i][j] - year2.values[i][j];
        }
    }
...
// A3.2 task 4 (Multiplication operator*)
Year operator*(float multiplication, Year& year)
{
    Year year_new;
    year_new.yearValue = year.yearValue;
    year_new.firstDayOfWeek = year.firstDayOfWeek;
    year_new.unit = year.unit;
    
    for(int i =0; i<days_in_year; i++ )
    {
        for(int j=0; j<interval_quater_hour; j++)
        {
            year_new.values[i][j] = multiplication * year.values[i][j];
        }
    }
    return year_new;
}
...
// A3.2 task 5
// define a function called set unit

void set_unit (Year& year, char* new_unit )
{
    year.unit = new_unit;
  ...
  do
    {
        ...
        switch (choice)
        {
            ...
            case 'm': total = (total - actual); break;  
            
            case 's': char actualORtotal_1;
                      float multi_value;
                      cout<<"a for actual\n"
                          <<"t for total"<<endl;
                      cin>> actualORtotal_1;
                      ...
                      if(actualORtotal_1 == 'a')
                      {
                        actual = (multi_value*actual);
                      }
                      else
                      total = (multi_value*total); break;

            case 'c': char actualORtotal;
                      cout<<"a for actual\n"
                          <<"t for total"<<endl;
                      cin>> actualORtotal;
                      ...
                      set_unit(total, new_unit); break;

            case 'v': cout << "sum actual = "<<sum(actual)<<" "<< actual.unit<<endl;
                      cout << "sum total  = "<< sum(total)<<" "<< total.unit<<endl; break;
